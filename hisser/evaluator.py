...
from hisser.func import functions, AGGOP_ALIAS
...
def filter_data(ds_list, max_points):
    ...
    for ds in filter(None, ds_list):
        trange = ds.end - ds.start

        if max_points == 1:
            datapoints = consolidate_dataset(ds, ds.data.shape[1])
        else:
            points_num = trange // ds.step
            if max_points < points_num:
                vals_per_point = math.ceil(points_num / max_points)
                newstep = int(vals_per_point * ds.step)
                newstart = (ds.start + newstep - 1) // newstep * newstep
                offset = (newstart - ds.start) // ds.step
                datapoints = consolidate_dataset(ds, vals_per_point, offset, newstart)
            else:
                datapoints = consolidate_dataset(ds, 1)

        for name, points in datapoints:
            series_data.append(dict(target=name.name, tags=name.tags, datapoints=points))

    return series_data


def consolidate_dataset(ds, values_per_point, offset=0, start=None):
    fname = AGGOP_ALIAS[ds.consolidate or 'mean']
    if start is None:
        start = ds.start
    for (name, i) in ds.names:
        if values_per_point == 1:
            yield name, Datapoints(ds.data[i,:], start, ds.step)
        else:
            yield name, Datapoints(aggop.op_window(fname, ds.data[i,offset:], values_per_point, 0),
                                   start,
                                   ds.step*values_per_point)
